...
/*
void draw_button_frame_2 (SDL_Renderer* renderer, int x, int y){
    SDL_Texture* background = loadTextture("END.png", renderer);
    SDL_RenderCopy( renderer, background, NULL, NULL);
...
int choose_options_2(SDL_Renderer* renderer, int choice, int &x, int &y){
    ...
    while (true) {
        ...
        draw_button_frame_2(renderer, x, y);
        SDL_RenderPresent(renderer);
        if (e.type == SDL_MOUSEBUTTONDOWN) {
            if (e.button.button == SDL_BUTTON_LEFT)
                cout << "mouse coordinates  " << e.button.x << " " << e.button.y << endl;
